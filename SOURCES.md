...
| **Verdict** | **cannot be run on `PRJNA788522` at all: it requires PAIRED-END reads and those arms are single-end 151 nt.** `Extract.py` pairs reads *by read id* -- each read waits in a `seen` dict and is written only when the same id appears a second time (lines 138-154) -- so on a single-end library both FASTQs stay empty, `bwa mem` aligns nothing and the extracted BAM has 0 records, after the log has reported "Working 100 %". Measured: 901,938 reads in, 0 out. Its own description says "paired-end sequencing NGS libraries"; this is an input-shape mismatch and not a configuration error |
| Provenance | reference (published method), not data; no call sets were produced |
